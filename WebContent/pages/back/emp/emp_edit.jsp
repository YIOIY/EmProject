...
<script type="text/javascript" src="js/pages/back/emp/emp_edit.js"></script>
</head>
								...
								<div class="panel-body" style="height : 95%;">
									<form action="<%=editEmpUrl%>" id="myform" method="post" class="form-horizontal" enctype="multipart/form-data">
											...
											<div class="col-md-5">
												<select id="emp.lid" name="emp.lid" class="form-control">
													<c:forEach items="${allLevels}" var="level">
														<option value="${level.lid}" ${emp.lid==level.lid?"selected" : ""}>${level.title}-${level.flag}（${level.losal} ~ ${level.hisal}）</option>
													</c:forEach>
												</select>
											</div> 
										...
										<div class="form-group" id="photoDiv">
											...
											<div class="col-md-5">
												<input type="file" name="photo" id="photo" class="form-control input-sm" placeholder="如果要修改员工照片，可以重新上传">
											</div>
